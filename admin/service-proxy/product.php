...
class Product extends Rest {
    ...
    public function getOne(){
        ...
        while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            $feature_stmt = $this->dbConn->prepare("SELECT feature_id FROM product_feature WHERE product_id={$result['id']}");
            $feature_stmt->execute();
            while($f = $feature_stmt->fetch(PDO::FETCH_ASSOC)){
                $fetch_data = $this->dbConn->prepare("SELECT id, name, name_en, group_id FROM features WHERE id={$f['feature_id']}");
                $fetch_data->execute();
                while($row = $fetch_data->fetch(PDO::FETCH_ASSOC)){
                    $featuresarray['id'] = $row['id'];
                    $featuresarray['name'] = $row['name'];
                    $featuresarray['group_id'] = $row['group_id'];
                    $featuresarray['group'] = new stdClass();
                    $groups = $this->dbConn->prepare("SELECT id, name FROM features_group WHERE id={$row['group_id']}");
                    $groups->execute();
                    while($row1 = $groups->fetch(PDO::FETCH_ASSOC)){
                        $featuresarray['group']->id = $row1['id'];
					    $featuresarray['group']->name = $row1['name'];
                    }
                }
                array_push($result['features'], $featuresarray);
            }
            return $result;
        }   
    ...
    public function edit() { 
        ...
        $sql = "UPDATE products SET 
            ...
            $deletefetures = $this->dbConn->prepare("DELETE FROM product_feature WHERE product_id={$data->id}");
            $deletefetures->execute();
            foreach($data->features as $key){
                $insertfeature = $this->dbConn->prepare("INSERT INTO product_feature VALUES (null, {$data->id},{$key})");
    ...
    public function add() {
        ...
        foreach($arr->features as $key){
            $insertfeature = $this->dbConn->prepare("INSERT INTO product_feature VALUES (null, {$last_id},{$key})");
            $insertfeature->execute();
        }
